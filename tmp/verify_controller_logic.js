// Standing Mock Test for TransactionalController.renderPreview extraction logic
function testExtraction(document) {
    ...
    if (Array.isArray(trigger.items) && trigger.items.length > 0) {
        ...
        items = qboLines
            .filter((l) => l.DetailType === 'SalesItemLineDetail' || l.Amount)
            .map((l, i) => ({
                id: i + 1,
